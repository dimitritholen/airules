...
def main():
    """Main test runner."""
    parser = argparse.ArgumentParser(description="Comprehensive test runner for airules")
    parser.add_argument("--suite", choices=["all", "unit", "integration", "performance", "error"], 
                       default="all", help="Test suite to run")
    parser.add_argument("--coverage", action="store_true", help="Generate coverage report")
    parser.add_argument("--benchmark", action="store_true", help="Run performance benchmarks")
    parser.add_argument("--verbose", "-v", action="store_true", help="Verbose output")
    parser.add_argument("--parallel", "-j", type=int, help="Number of parallel test workers")
    ...
    test_configs = {
        "unit": {
            ...
            "command": base_cmd + ["tests/test_airules.py", "tests/test_file_creation.py", "tests/test_venv_check.py"],
            "required": True
        },
        "integration": {
            "description": "Integration Tests",
            "command": base_cmd + ["tests/test_auto_integration.py", "-m", "integration"],
            "required": True
        },
        "performance": {
            "description": "Performance Tests",
            "command": base_cmd + ["tests/test_performance.py", "-m", "performance"] + 
                      (["--benchmark-only"] if args.benchmark else ["--benchmark-skip"]),
            "required": False
        },
        "error": {
            "description": "Error Handling Tests",
            "command": base_cmd + ["tests/test_error_handling.py", "-m", "error_handling"],
            "required": True
        }
    }
    ...
    if args.coverage:
        coverage_result = run_command(coverage_config["command"], coverage_config["description"])
