...
class PackageParser:
    ...
    def parse_npm(self, file_path: str) -> PackageInfo:
        ...
        for name, version in data.get('devDependencies', {}).items():
            dev_dependencies.append(DependencyInfo(name=name, version=version, is_dev=True))
        
        # Parse optional dependencies
        for name, version in data.get('optionalDependencies', {}).items():
            dependencies.append(DependencyInfo(name=name, version=version, is_optional=True))
    ...
    def parse_requirements_txt(self, file_path: str) -> PackageInfo:
        ...
        is_dev_file = any(keyword in Path(file_path).name.lower() 
    ...
    def parse_pyproject_toml(self, file_path: str) -> PackageInfo:
        ...
        for name, version_info in poetry.get('dependencies', {}).items():
            ...
            if isinstance(version_info, str):
                ...
                dependencies.append(DependencyInfo(name=name, version=version, is_optional=is_optional))
        
        # Parse poetry dev dependencies
        for name, version_info in poetry.get('group', {}).get('dev', {}).get('dependencies', {}).items():
            if isinstance(version_info, str):
                dev_dependencies.append(DependencyInfo(name=name, version=version_info, is_dev=True))
            elif isinstance(version_info, dict):
    ...
    def parse_pipfile(self, file_path: str) -> PackageInfo:
        ...
        return PackageInfo(
            ...
            metadata={
                ...
                'python_full_version': data.get('requires', {}).get('python_full_version')
            }
        )
    ...
    def parse_gemfile_lock(self, file_path: str) -> PackageInfo:
        ...
        with open(file_path, 'r', encoding='utf-8') as f:
            for line in f:
                ...
                if current_section == 'gems' and line and not line.startswith('remote:'):
    ...
    def _parse_python_requirement(self, requirement: str) -> Optional[DependencyInfo]:
        ...
        if any(proto in original_requirement.lower() for proto in ['http://', 'https://', 'git+', 'ssh://']):
