...
class CodebaseAnalyzer:
    ...
    def analyze(self, project_path: str) -> AnalysisResult:
        ...
        try:
            ...
            if not path_obj.is_dir():
                raise FileOperationError(f"Project path is not a directory: {project_path}")
            
            # Scan directory structure
            files_by_extension, structure, file_stats = self.file_scanner.scan_directory(project_path)
            ...
            logger.info(f"Analysis complete. Found {len(languages)} languages, "
    ...
    def get_project_summary(self, project_path: str) -> str:
        ...
        try:
            ...
            if structure.source_directories:
                lines.append(f"  • Source dirs: {', '.join(structure.source_directories[:3])}")
            if structure.test_directories:
                lines.append(f"  • Test dirs: {', '.join(structure.test_directories[:3])}")
            lines.append("")
    ...
    def is_valid_project(self, project_path: str) -> bool:
        ...
        try:
            ...
            files_by_extension, _, stats = self.file_scanner.scan_directory(project_path)
            
            # Consider it valid if it has any code files or common project files
            return (
                stats.code_files > 0 or 
                stats.config_files > 0 or
                any(ext in ['.md', '.txt', '.rst'] for ext in files_by_extension.keys())
            )
    ...
    def filter_significant_languages(self, languages: List[LanguageInfo]) -> List[LanguageInfo]:
        """
