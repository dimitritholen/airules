...
class LanguageDetector:
    ...
    EXTENSION_MAP = {
        ...
        '.R': 'R',
        '.jl': 'Julia',
        '.m': 'MATLAB',  # Can conflict with Objective-C
        '.sql': 'SQL',
        '.json': 'JSON',
        '.xml': 'XML',
        '.yaml': 'YAML',
    ...
    def __init__(self):
        ...
        for lang, patterns in self.CONTENT_PATTERNS.items():
            self._compiled_patterns[lang] = [re.compile(pattern, re.MULTILINE) for pattern in patterns]

    def detect_languages(self, files_by_extension: Dict[str, List[str]], 
                        file_scanner=None) -> List[LanguageInfo]:
        """
    ...
    def _refine_with_content_analysis(self, language_counts: Dict[str, int],
        ...
        if 'MATLAB' in language_counts and 'Objective-C' in language_counts:
            m_files = language_files.get('MATLAB', []) + language_files.get('Objective-C', [])
        ...
        for file_path, candidate_langs in ambiguous_files:
            ...
            if content:
                detected_lang = self._detect_language_from_content(content, candidate_langs)
                if detected_lang:
                    # Adjust counts
    ...
    def _detect_language_from_content(self, content: str, 
                                    candidate_languages: List[str]) -> Optional[str]:
        """
    ...
    def get_primary_language(self, languages: List[LanguageInfo]) -> Optional[LanguageInfo]:
        """
        ...
        code_languages = [
            ...
            if lang.name not in {'HTML', 'CSS', 'JSON', 'YAML', 'XML', 'Markdown', 
                               'TOML', 'Dockerfile', 'reStructuredText', 'LaTeX'}
        ]
        ...
        if primary.confidence >= 0.1 or (
            len(code_languages) > 1 and 
            primary.confidence > code_languages[1].confidence * 2
        ):
            return primary
        
        return None

    def detect_frameworks_and_libraries(self, files_by_extension: Dict[str, List[str]], 
                                      file_scanner=None) -> List[str]:
        """
    ...
    def is_language_compatible(self, lang1: str, lang2: str) -> bool:
        ...
        """
        return (
            lang2 in self.RELATED_LANGUAGES.get(lang1, []) or
            lang1 in self.RELATED_LANGUAGES.get(lang2, [])
        )