...
class TagGenerator:
    ...
    def generate_tags(
        ...
        tag_sources = {
            ...
            'project_type': self._generate_project_type_tags(analysis_result.project_type),
            ...
            'deployment': self._generate_deployment_tags(analysis_result.deployment_info),
        ...
        if len(final_tags) < MIN_TAGS_PER_PROJECT:
            suggestions = self._generate_additional_suggestions(analysis_result, final_tags)
            final_tags.extend(suggestions[:MIN_TAGS_PER_PROJECT - len(final_tags)])
        
        return final_tags[:MAX_TAGS_PER_PROJECT]
    
    def _generate_framework_tags(self, frameworks: List[FrameworkInfo]) -> List[Tuple[str, float]]:
    ...
    def _generate_language_tags(self, language_info: LanguageInfo) -> List[Tuple[str, float]]:
    ...
    def _generate_project_type_tags(self, project_type: ProjectType) -> List[Tuple[str, float]]:
    ...
    def _generate_directory_tags(self, directory_info: DirectoryInfo) -> List[Tuple[str, float]]:
    ...
    def _generate_testing_tags(self, testing_info: TestingInfo) -> List[Tuple[str, float]]:
    ...
    def _generate_security_tags(self, security_info: SecurityInfo) -> List[Tuple[str, float]]:
    ...
    def _generate_deployment_tags(self, deployment_info: DeploymentInfo) -> List[Tuple[str, float]]:
    ...
    def _generate_context_tags(self, analysis_result: AnalysisResult) -> List[Tuple[str, float]]:
    ...
    def _generate_architectural_tags(self, analysis_result: AnalysisResult) -> List[Tuple[str, float]]:
        ...
        # API-focused project
        api_frameworks = [fw for fw in analysis_result.frameworks 
                         if fw.category in [FrameworkCategory.WEB_FRAMEWORK, FrameworkCategory.API]]
        if api_frameworks and not any(fw.category == FrameworkCategory.FRONTEND for fw in analysis_result.frameworks):
            tags.append(('api-focused', 0.8))
            tags.append(('backend', 0.9))
        
        return tags
    
    def _analyze_testing_context(self, analysis_result: AnalysisResult) -> List[Tuple[str, float]]:
        """Analyze testing-related context."""
        tags = []
        
        # High test coverage suggests TDD/BDD
        if analysis_result.testing_info.test_coverage_tools:
            if analysis_result.testing_info.has_unit_tests and analysis_result.testing_info.has_integration_tests:
    ...
    def _analyze_api_context(self, analysis_result: AnalysisResult) -> List[Tuple[str, float]]:
        ...
        for framework in analysis_result.frameworks:
            if any(indicator in framework.name.lower() for indicator in rest_indicators):
    ...
    def _analyze_web_context(self, analysis_result: AnalysisResult) -> List[Tuple[str, float]]:
        """Analyze web development context."""
        tags = []
        
        # Frontend frameworks
        frontend_frameworks = [fw for fw in analysis_result.frameworks 
    ...
    def _analyze_mobile_context(self, analysis_result: AnalysisResult) -> List[Tuple[str, float]]:
    ...
    def _analyze_data_context(self, analysis_result: AnalysisResult) -> List[Tuple[str, float]]:
        """Analyze data processing context."""
        tags = []
        
        data_frameworks = ['pandas', 'numpy', 'scikit-learn', 'tensorflow', 'pytorch']
        data_count = sum(1 for fw in analysis_result.frameworks 
                        if fw.name.lower() in data_frameworks)
    ...
    def _analyze_security_context(self, analysis_result: AnalysisResult) -> List[Tuple[str, float]]:
    ...
    def _analyze_deployment_context(self, analysis_result: AnalysisResult) -> List[Tuple[str, float]]:
        """Analyze deployment context."""
        tags = []
        
        if analysis_result.deployment_info.ci_cd_tools and analysis_result.deployment_info.containerized:
            tags.append(('modern-deployment', 0.8))
            tags.append(('devops', 0.8))
        
        return tags
    
    def _analyze_architecture_context(self, analysis_result: AnalysisResult) -> List[Tuple[str, float]]:
        """Analyze architectural patterns."""
        tags = []
        
        # MVC pattern indicators
        mvc_indicators = ['django', 'rails', 'spring', 'laravel']
        if any(indicator in fw.name.lower() for fw in analysis_result.frameworks for indicator in mvc_indicators):
            tags.append(('mvc', 0.8))
    ...
    def _prioritize_tags(self, tag_scores: Dict[str, float], analysis_result: AnalysisResult) -> List[str]:
        ...
        filtered_tags = [
            tag for tag, score in sorted_tags 
    ...
    def _generate_additional_suggestions(
        ...
        if not any(tag in current_set for tag in ['frontend', 'backend', 'fullstack', 'cli', 'library']):
        ...
        if 'documentation' not in current_set and analysis_result.directory_info.has_docs_dir:
    ...
    def get_tag_explanations(self, tags: List[str], analysis_result: AnalysisResult) -> Dict[str, str]:
        ...
        for tag in tags:
            ...
            if tag == analysis_result.languages.primary_language.lower():
                explanation.append(f"Primary programming language")
            elif tag in analysis_result.languages.languages:
                explanation.append(f"Secondary programming language")
            
            # Check language mapping
            if tag in LANGUAGE_TAGS.get(analysis_result.languages.primary_language.lower(), []):
                explanation.append(f"Related to {analysis_result.languages.primary_language} programming")
            
            # Check project type
            if tag == analysis_result.project_type.value:
                explanation.append(f"Matches project type")
            elif tag in PROJECT_TYPE_TAGS.get(analysis_result.project_type, []):
                explanation.append(f"Relevant for {analysis_result.project_type.value} projects")
            ...
            # Check testing info
            if tag in ["unit-tests", "integration-tests", "e2e"] and analysis_result.testing_info.has_unit_tests:
