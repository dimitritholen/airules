...
from .package_parser import PackageParser, PackageInfo, DependencyInfo
...
class FrameworkDetector:
    ...
    FRAMEWORK_DEFINITIONS = {
        ...
        'express': {
            ...
            'file_patterns': [r'require\(["\']express["\']\)', r'from ["\']express["\']'],
        },
        'nestjs': {
            'name': 'NestJS',
            'type': FrameworkType.BACKEND_FRAMEWORK,
        ...
        'fastapi': {
            ...
            'file_patterns': [r'from fastapi', r'import fastapi', r'@app\.(get|post|put|delete)'],
        },
        'tornado': {
            'name': 'Tornado',
            'type': FrameworkType.WEB_FRAMEWORK,
        ...
        'spring': {
            ...
            'type': FrameworkType.WEB_FRAMEWORK,
            'language': 'java',
            'dependencies': ['org.springframework:spring-core', 'org.springframework:spring-web'],
    ...
    def _detect_from_dependencies(self, package_info: PackageInfo) -> List[FrameworkInfo]:
    ...
    def _detect_from_files(self, project_path: Path) -> List[FrameworkInfo]:
        ...
        source_files = []
        for ext in ['.js', '.ts', '.jsx', '.tsx', '.py', '.java', '.rs', '.go', '.php', '.rb', '.vue', '.svelte']:
            ...
            if confidence >= 0.5:  # At least 50% of structure matches
                if framework_key in self.FRAMEWORK_DEFINITIONS:
                    ...
                    framework_info = FrameworkInfo(
                        ...
                        type=framework_def['type'],
                        language=framework_def['language'],
                        confidence=confidence * 0.6,  # Lower confidence for structure-based detection
    ...
    def _deduplicate_frameworks(self, frameworks: List[FrameworkInfo]) -> List[FrameworkInfo]:
    ...
    def _determine_project_type(self, frameworks: List[FrameworkInfo]) -> str:
        ...
        for framework in frameworks:
            if framework.type in [FrameworkType.WEB_FRAMEWORK, FrameworkType.FRONTEND_FRAMEWORK, FrameworkType.BACKEND_FRAMEWORK]:
            ...
            elif framework.type in [FrameworkType.ML_FRAMEWORK, FrameworkType.DATA_PROCESSING]:
    ...
    def _extract_deployment_targets(self, frameworks: List[FrameworkInfo]) -> List[str]:
        ...
        for framework in frameworks:
            ...
            elif framework.type in [FrameworkType.WEB_FRAMEWORK, FrameworkType.BACKEND_FRAMEWORK]:
    ...
    def get_framework_suggestions(self, project_path: str) -> Dict[str, List[str]]:
        ...
        if 'javascript' in languages and not any('jest' in name or 'mocha' in name for name in detected_names):
        ...
        if 'javascript' in languages and not any(tool in detected_names for tool in ['webpack', 'vite', 'rollup']):
        ...
        if any('react' in name for name in detected_names):
            if not any(ui in detected_names for ui in ['material-ui', 'antd', 'chakra']):
                suggestions['ui_libraries'].extend(['Material-UI', 'Ant Design', 'Chakra UI'])
            if not any(state in detected_names for state in ['redux', 'mobx', 'zustand']):
                suggestions['state_management'].extend(['Redux Toolkit', 'Zustand'])
        
        # Database suggestions
        if any(fw.type in [FrameworkType.WEB_FRAMEWORK, FrameworkType.BACKEND_FRAMEWORK] for fw in frameworks):
        ...
        if not any(fw.type in [FrameworkType.CONTAINER, FrameworkType.DEPLOYMENT] for fw in frameworks):
    ...
    def analyze_compatibility(self, frameworks: List[FrameworkInfo]) -> Dict[str, any]:
        ...
        conflicts = [
            ...
            (['Express.js', 'Fastify', 'NestJS'], 'Multiple Node.js frameworks detected'),
        ]
        
        framework_names = [f.name for f in frameworks]
        
        for conflict_group, message in conflicts:
            detected_in_group = [name for name in framework_names if name in conflict_group]
        ...
        if any(f.name == 'React' for f in frameworks):
            if not any(f.type == FrameworkType.BUILD_TOOL for f in frameworks):
                recommendations.append('Consider adding a build tool like Vite or webpack')
