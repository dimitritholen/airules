...
class DependencyAnalyzer:
    ...
    def analyze_project_dependencies(self, project_path: str) -> ProjectDependencyAnalysis:
        ...
        for package_info in package_infos:
            ...
            for dep in package_info.dependencies:
                report = self._analyze_single_dependency(dep, package_info.language, False)
                all_dependency_reports.append(report)
                total_vulnerabilities += len(report.vulnerabilities)
                if report.health in [DependencyHealth.OUTDATED, DependencyHealth.DEPRECATED]:
                    total_outdated += 1
                    
            # Analyze dev dependencies
            for dep in package_info.dev_dependencies:
                report = self._analyze_single_dependency(dep, package_info.language, True)
                all_dependency_reports.append(report)
                total_vulnerabilities += len(report.vulnerabilities)
                if report.health in [DependencyHealth.OUTDATED, DependencyHealth.DEPRECATED]:
                    total_outdated += 1
    ...
    def _analyze_single_dependency(self, dep: DependencyInfo, language: str, is_dev: bool) -> DependencyReport:
    ...
    def _determine_health_status(self, package_name: str, version: str, language: str) -> DependencyHealth:
    ...
    def _is_likely_outdated(self, package_name: str, version: str, language: str) -> bool:
        ...
        if version:
            ...
            if clean_version:
                try:
                    ...
                    popular_packages = ['react', 'vue', 'angular', 'django', 'flask', 'express']
    ...
    def _deduplicate_dependency_reports(self, reports: List[DependencyReport]) -> List[DependencyReport]:
    ...
    def _generate_recommendations(self, reports: List[DependencyReport], package_infos: List[PackageInfo]) -> List[str]:
        ...
        deprecated_deps = [r for r in reports if r.health == DependencyHealth.DEPRECATED]
        if deprecated_deps:
            recommendations.append(f"Replace {len(deprecated_deps)} deprecated dependencies")
            for dep in deprecated_deps[:3]:  # Show top 3
                replacement = self.DEPRECATED_PACKAGES.get(dep.name)
                if replacement:
                    recommendations.append(f"  - Replace '{dep.name}' with {replacement}")
        ...
        has_security_linter = any(
            'eslint' in r.name.lower() or 'bandit' in r.name.lower() or 'safety' in r.name.lower()
            for r in reports
        )
        if not has_security_linter:
            recommendations.append("Add security linting tools (ESLint, Bandit, Safety)")
        
        # Check for missing testing frameworks
        has_testing = any(
            r.name.lower() in ['jest', 'mocha', 'pytest', 'junit']
            for r in reports
        )
        if not has_testing:
    ...
    def _analyze_frameworks(self, project_path: str, dependency_reports: List[DependencyReport]) -> Dict:
    ...
    def check_license_compatibility(self, reports: List[DependencyReport], project_license: str = 'MIT') -> Dict:
        ...
        for report in reports:
            if report.license:
                ...
                license_distribution[report.license] = license_distribution.get(report.license, 0) + 1
    ...
    def export_report(self, analysis: ProjectDependencyAnalysis, format: str = 'json') -> str:
