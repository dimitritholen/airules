...
class TestScalabilityTests:
    ...
    def test_scalability_by_file_count(self, tmp_path, file_count):
        ...
        for i in range(file_count):
            file_path = project_path / "src" / f"scale_test_{file_count}" / f"file_{i}.py"
            file_path.parent.mkdir(parents=True, exist_ok=True)
        ...
        assert analysis_time < (file_count * 0.01)  # Should be much faster than 10ms per file
    ...
    def test_memory_scalability(self, tmp_path):
        ...
        for size in [10, 50, 100]:
            ...
            # Create files
            for i in range(size):
                file_path = project_path / "src" / f"memory_scale_{size}" / f"file_{i}.py"
                file_path.parent.mkdir(parents=True, exist_ok=True)
                file_path.write_text("def function(): pass\n" * 100)
