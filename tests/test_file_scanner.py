...
from airules.analyzer.models import ProjectStructure, FileStats
...
class TestFileScanner:
    ...
    def test_should_ignore_dir(self):
        ...
        for dirname, expected in test_cases:
            ...
            assert result == expected, f"Failed for {dirname}: expected {expected}, got {result}"
    ...
    def test_get_file_content_sample_success(self):
        ...
        with tempfile.NamedTemporaryFile(mode='w', suffix='.py', delete=False) as tmp_file:
            ...
            try:
                content = self.scanner.get_file_content_sample(tmp_file.name, max_lines=2)
    ...
    def test_get_file_content_sample_max_lines(self):
        ...
        with tempfile.NamedTemporaryFile(mode='w', suffix='.py', delete=False) as tmp_file:
            ...
            try:
                content = self.scanner.get_file_content_sample(tmp_file.name, max_lines=5)
                lines = content.split('\n')
                assert len(lines) == 5
    ...
    def test_analyze_structure_typical_project(self):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            structure = self.scanner._analyze_structure(tmp_path, all_files, test_files, config_files)
    ...
    def test_calculate_stats_comprehensive(self):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            stats = self.scanner._calculate_stats(all_files, test_files, config_files, doc_files)
