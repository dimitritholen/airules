    ...
    def test_permission_denied_directory(self, tmp_path):
        ...
        try:
            ...
            with runner.isolated_filesystem(temp_dir=tmp_path):
                ...
                # The auto command should handle permission errors gracefully
                pass
        finally:
...
class TestAPIErrorHandling:
    ...
    def test_openai_api_error(self, tmp_path):
        ...
        with patch("airules.api_clients.AIClientFactory.get_client") as mock_get_client:
            mock_client = Mock()
            mock_client.generate_completion.side_effect = APIError("OpenAI API error: Rate limit exceeded")
    ...
    def test_perplexity_api_error(self, tmp_path):
        ...
        with patch("airules.api_clients.AIClientFactory.get_research_client") as mock_get_research_client:
            mock_research_client = Mock()
            mock_research_client.generate_completion.side_effect = APIError("Perplexity API error: Service unavailable")
    ...
    def test_network_timeout(self, tmp_path):
        ...
        with patch("airules.api_clients.AIClientFactory.get_client") as mock_get_client:
            import requests
            mock_client = Mock()
            mock_client.generate_completion.side_effect = requests.exceptions.Timeout("Request timed out")
            mock_get_client.return_value = mock_client
    ...
    def test_network_connection_error(self, tmp_path):
        ...
        with patch("airules.api_clients.AIClientFactory.get_client") as mock_get_client:
            import requests
            mock_client = Mock()
            mock_client.generate_completion.side_effect = requests.exceptions.ConnectionError("Connection failed")
            mock_get_client.return_value = mock_client
...
class TestConfigurationErrorHandling:
    ...
    def test_invalid_tool_specification(self, tmp_path):
        ...
        with runner.isolated_filesystem(temp_dir=project_path.parent):
            ...
            content = content.replace("tools = cursor,claude,copilot,cline,roo", "tools = invalid_tool")
...
class TestEdgeCases:
    ...
    def test_mixed_language_project(self, tmp_path):
        ...
        python_dir.mkdir()
        (python_dir / "app.py").write_text("from flask import Flask\napp = Flask(__name__)")
        ...
        (project_path / "main.rs").write_text("fn main() { println!(\"Hello, world!\"); }")
...
""")
    ...
    def test_project_with_unusual_file_extensions(self, tmp_path):
        ...
        (project_path / "Makefile").write_text("install:\n\tpip install -r requirements.txt")
...
""")
    ...
    def test_unicode_and_special_characters(self, tmp_path):
        ...
        unicode_file.write_text("# Файл с unicode именем\ndef функция():\n    return 'Привет мир'")
