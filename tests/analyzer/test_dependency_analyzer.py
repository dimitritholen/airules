...
class TestDependencyAnalyzer:
    ...
    def test_analyze_project_dependencies_basic(self):
        ...
        package_info = self.create_package_info("javascript", ["react", "lodash"], ["jest"])
        
        with patch.object(self.analyzer.package_parser, 'parse_all_package_files', return_value=[package_info]):
    ...
    def test_analyze_project_no_packages(self):
        ...
        with patch.object(self.analyzer.package_parser, 'parse_all_package_files', return_value=[]):
            analysis = self.analyzer.analyze_project_dependencies(str(self.temp_dir))
    ...
    def test_vulnerability_detection(self):
        ...
        report = self.analyzer._analyze_single_dependency(vulnerable_dep, "javascript", False)
    ...
    def test_deprecated_package_detection(self):
        ...
        report = self.analyzer._analyze_single_dependency(deprecated_dep, "javascript", False)
        
        assert report.health == DependencyHealth.DEPRECATED
        assert len(report.vulnerabilities) > 0
        assert any("deprecated" in vuln.title.lower() for vuln in report.vulnerabilities)
    
    def test_health_status_determination(self):
        """Test dependency health status determination."""
        # Healthy dependency
        healthy_dep = DependencyInfo(name="unknown-package", version="1.0.0")
        health = self.analyzer._determine_health_status("unknown-package", "1.0.0", "javascript")
        assert health == DependencyHealth.HEALTHY
        
        # Deprecated dependency
        deprecated_health = self.analyzer._determine_health_status("moment", "2.29.0", "javascript")
        assert deprecated_health == DependencyHealth.DEPRECATED
        
        # Vulnerable dependency
        vulnerable_health = self.analyzer._determine_health_status("lodash", "4.17.11", "javascript")
    ...
    def test_outdated_detection_heuristic(self):
        ...
        assert self.analyzer._is_likely_outdated("react", "0.14.0", "javascript") is True
        assert self.analyzer._is_likely_outdated("react", "1.0.0", "javascript") is True
        assert self.analyzer._is_likely_outdated("react", "18.0.0", "javascript") is False
        
        # Unknown package should not be flagged as outdated
        assert self.analyzer._is_likely_outdated("unknown-package", "0.1.0", "javascript") is False
    ...
    def test_health_score_calculation(self):
        ...
        reports = [
            DependencyReport("healthy", "1.0.0", "javascript", health=DependencyHealth.HEALTHY),
            DependencyReport("outdated", "1.0.0", "javascript", health=DependencyHealth.OUTDATED),
            DependencyReport("vulnerable", "1.0.0", "javascript", health=DependencyHealth.VULNERABLE, 
                           vulnerabilities=[VulnerabilityInfo("CVE-123", SecurityRisk.HIGH, "Test", "Test vuln", "*")]),
            DependencyReport("dev-tool", "1.0.0", "javascript", health=DependencyHealth.HEALTHY, is_dev_dependency=True),
        ]
    ...
    def test_generate_recommendations(self):
        ...
        reports = [
            DependencyReport("vulnerable", "1.0.0", "javascript", health=DependencyHealth.VULNERABLE,
                           vulnerabilities=[VulnerabilityInfo("CVE-123", SecurityRisk.HIGH, "Test", "Test", "*")]),
            DependencyReport("moment", "2.29.0", "javascript", health=DependencyHealth.DEPRECATED),
            DependencyReport("outdated", "1.0.0", "javascript", health=DependencyHealth.OUTDATED),
        ]
        
        package_infos = [self.create_package_info("javascript", ["vulnerable", "moment", "outdated"])]
        recommendations = self.analyzer._generate_recommendations(reports, package_infos)
    ...
    def test_framework_analysis_integration(self):
        ...
        with patch.object(self.analyzer.package_parser, 'parse_all_package_files', return_value=[package_info]):
            with patch.object(self.analyzer.framework_detector, 'detect_frameworks') as mock_detect:
                from airules.analyzer.framework_detector import FrameworkInfo, FrameworkType
    ...
    def test_license_compatibility_checking(self):
        ...
        reports = [
            ...
            DependencyReport("apache-package", "1.0.0", "javascript", license="Apache-2.0"),
        ...
        assert any("GPL-3.0" in issue["license"] for issue in compatibility["compatibility_issues"])
    ...
    def test_export_report_json(self):
        ...
        analysis = ProjectDependencyAnalysis(
            ...
            health_score=0.8,
            dependency_reports=[
                DependencyReport("test-package", "1.0.0", "javascript", 
                               vulnerabilities=[VulnerabilityInfo("CVE-123", SecurityRisk.MEDIUM, "Test", "Test vuln", "*")])
            ],
    ...
    def test_export_report_markdown(self):
        ...
        analysis = ProjectDependencyAnalysis(
            ...
            health_score=0.8,
            dependency_reports=[
                DependencyReport("vulnerable-package", "1.0.0", "javascript",
                               vulnerabilities=[VulnerabilityInfo("CVE-123", SecurityRisk.HIGH, "Critical Bug", "Test vuln", "*")])
            ],
    ...
    def test_dependency_report_dataclass(self):
        ...
        vuln = VulnerabilityInfo("CVE-123", SecurityRisk.MEDIUM, "Test", "Test vuln", "*")
    ...
    @pytest.mark.parametrize("security_risk", [
    ...
    def test_dependency_health_enum(self, health_status):
        ...
        assert health_status.value in ["healthy", "outdated", "deprecated", "vulnerable", "abandoned"]