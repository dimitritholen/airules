...
from airules.analyzer.package_parser import PackageInfo, DependencyInfo
...
class TestFrameworkDetector:
    ...
    def create_package_info(self, language: str, dependencies: list, dev_dependencies: list = None) -> PackageInfo:
        ...
        return PackageInfo(
            ...
            dependencies=[DependencyInfo(name=dep, version="1.0.0") for dep in dependencies],
            dev_dependencies=[DependencyInfo(name=dep, version="1.0.0", is_dev=True) for dep in dev_dependencies],
            scripts={},
            metadata={}
        )
    ...
    def test_detect_react_from_dependencies(self):
        ...
        with patch.object(self.detector.package_parser, 'parse_all_package_files', return_value=[package_info]):
    ...
    def test_detect_django_from_dependencies(self):
        ...
        package_info = self.create_package_info("python", ["django", "djangorestframework"])
    ...
    def test_detect_multiple_frameworks(self):
        ...
        with patch.object(self.detector.package_parser, 'parse_all_package_files', return_value=[js_package, py_package]):
            frameworks = self.detector.detect_frameworks(str(self.temp_dir))
    ...
    def test_detect_frameworks_from_files(self):
        ...
        self.create_temp_file("src/App.jsx", """
...
""")
        ...
        self.create_temp_file("public/index.html", "<html><body><div id='root'></div></body></html>")
        ...
        assert "file_presence" in detection_methods or "file_pattern" in detection_methods
    
    def test_detect_vue_from_files(self):
        """Test detecting Vue.js from .vue files."""
        self.create_temp_file("src/App.vue", """
<template>
  <div id="app">
    <h1>Hello Vue!</h1>
  </div>
...
""")
    ...
    def test_detect_angular_from_files(self):
        ...
        self.create_temp_file("angular.json", json.dumps({
            "version": 1,
            "projects": {
                "my-app": {
        ...
        self.create_temp_file("src/app/app.component.ts", """
...
""")
    ...
    def test_detect_go_frameworks(self):
        ...
        package_info = self.create_package_info("go", ["github.com/gin-gonic/gin", "gorm.io/gorm"])
    ...
    def test_detect_testing_frameworks(self):
        ...
        testing_frameworks = [f for f in frameworks if f.type == FrameworkType.TESTING_FRAMEWORK]
    ...
    def test_detect_build_tools(self):
        ...
        package_info = self.create_package_info("javascript", [], ["webpack", "vite", "rollup"])
    ...
    def test_detect_ui_libraries(self):
        ...
        package_info = self.create_package_info("javascript", ["@mui/material", "antd", "bootstrap"])
    ...
    def test_detect_mobile_frameworks(self):
        ...
        with patch.object(self.detector.package_parser, 'parse_all_package_files', return_value=[rn_package]):
            frameworks = self.detector.detect_frameworks(str(self.temp_dir))
        
        mobile_frameworks = [f for f in frameworks if f.type == FrameworkType.MOBILE_FRAMEWORK]
    ...
    def test_detect_container_deployment(self):
        ...
        self.create_temp_file("Dockerfile", """
FROM node:16
COPY . .
RUN npm install
CMD ["npm", "start"]
""")
        ...
        self.create_temp_file("docker-compose.yml", """
version: '3'
services:
  app:
    build: .
...
""")
        ...
        container_frameworks = [f for f in frameworks if f.type == FrameworkType.CONTAINER]
    ...
    def test_deduplicate_frameworks(self):
        ...
        frameworks = [
            FrameworkInfo("React", FrameworkType.FRONTEND_FRAMEWORK, "javascript", confidence=0.9),
            FrameworkInfo("React", FrameworkType.FRONTEND_FRAMEWORK, "javascript", confidence=0.7),
            FrameworkInfo("Vue.js", FrameworkType.FRONTEND_FRAMEWORK, "javascript", confidence=0.8),
        ]
    ...
    def test_get_project_technology(self):
        ...
        js_package = self.create_package_info("javascript", ["react", "express"], ["jest", "webpack"])
        ...
        assert tech_analysis.project_type in ['web', 'library', 'mobile', 'desktop', 'data_science', 'game', 'cli']
    ...
    def test_determine_project_type(self):
        ...
        web_frameworks = [
            FrameworkInfo("React", FrameworkType.FRONTEND_FRAMEWORK, "javascript", confidence=0.9),
            FrameworkInfo("Django", FrameworkType.WEB_FRAMEWORK, "python", confidence=0.8)
        ]
        ...
        # Mobile project
        mobile_frameworks = [
            FrameworkInfo("React Native", FrameworkType.MOBILE_FRAMEWORK, "javascript", confidence=0.9)
        ]
        ...
        ds_frameworks = [
            FrameworkInfo("Pandas", FrameworkType.DATA_PROCESSING, "python", confidence=0.9),
            FrameworkInfo("TensorFlow", FrameworkType.ML_FRAMEWORK, "python", confidence=0.8)
        ]
    ...
    def test_get_framework_suggestions(self):
        ...
        with patch.object(self.detector.package_parser, 'parse_all_package_files', return_value=[js_package]):
    ...
    def test_analyze_compatibility(self):
        ...
        conflict_issue = next((issue for issue in compatibility["issues"] if issue["type"] == "conflict"), None)
    ...
    def test_confidence_calculation(self):
        ...
        frameworks = [
            FrameworkInfo("React", FrameworkType.FRONTEND_FRAMEWORK, "javascript", confidence=0.9, detection_method="dependency"),
            FrameworkInfo("Jest", FrameworkType.TESTING_FRAMEWORK, "javascript", confidence=0.7, detection_method="file_pattern"),
            FrameworkInfo("Docker", FrameworkType.CONTAINER, "multi", confidence=0.8, detection_method="file_presence")
        ]
    ...
    def test_empty_project_detection(self):
        ...
        with patch.object(self.detector.package_parser, 'parse_all_package_files', return_value=[]):
            frameworks = self.detector.detect_frameworks(str(self.temp_dir))
    ...
    def test_large_project_file_limit(self):
        ...
        for i in range(150):  # More than the 100 file limit
            self.create_temp_file(f"src/component{i}.js", f"import React from 'react'; // Component {i}")
    ...
    @pytest.mark.parametrize("framework_name,expected_type", [
